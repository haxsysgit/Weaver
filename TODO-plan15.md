...
- [x] **5. Two-phase reading architecture** — **implemented 2026-08-08**: Locate (tool loop, model decides its own steps; router call and per-class budgets rejected by owner) → packet assembly (machinery, +/-20 expanded windows, notebook statements, judge verdict, tier-capped, ephemeral) → one toolless synthesis call (draft never persisted, only the final answer persists). Seam: run_turn packet_builder kwarg, src/weaver/retrieval/packet.py
