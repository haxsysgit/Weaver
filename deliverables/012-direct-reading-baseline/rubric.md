...
| Chapter 1 re-read from zero with three independent readers | Done | private notebook records; fresh three-role run |
| Chapters 2-100 read in groups of ten; later knowledge never leaks backwards | Done | private notebook and final reviewer ledger |
| Every entry: stable ID, kind, statement, supporting chapter + location, links, first-known chapter, later corrections | Done | 200 JSON statements; checker pass |
| Knowledge labelled confirmed_fact / character_belief / interpretation / theory / conflict | Done | schema validation; current slice uses confirmed facts and interpretations |
| Chapter JSON is the master; pages and connections rebuilt from it | Done | rebuild script and 924 connections |
| Readable notes and matching JSON for every chapter 1-100 | Done | 100 `chapters/` notes and 100 `reading/` records |
