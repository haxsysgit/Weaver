...
| Notebook checker built on synthetic data first | Pending | `scripts/check_story_notebook.py`, `tests/test_story_notebook_checker.py` |
| Checker rejects unsupported statements, bad locations, duplicate IDs, broken links, out-of-order or missing chapters, changed chapter files, copied prose in JSON/connections, unsafe perms, private text in Git | Pending | checker tests |
| Old chapter 1 attempt quarantined to private `old-attempts/` | Pending | results.md |
