...
map + judge, user position as soft preference, router + two-phase read,
skills.
...
- 2026-08-08: spoiler map = statement labels (1-1000) + verified volume
  ...
  judge is deterministic machinery, the synthesis call applies the
  framing.
- 2026-08-08: two-phase read = router (one call, 5 classes, picks skill
  pack / tier / budget / packet size), locate (2-6 tool calls), packet
  assembly (machinery, ephemeral 50-200K tokens), judge gate, one heavy
  toolless synthesis call, verify pass for transcendent only.
