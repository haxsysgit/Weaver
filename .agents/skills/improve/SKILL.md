...
**Hermes background subagents pitfall:** `delegate_task` returns immediately and child results arrive later. If you dispatch audit subagents, do not present a final audit as if their work was included until their summaries have actually re-entered the conversation. Either (a) continue direct auditing and clearly scope the report to your own verified reads, or (b) wait for/handle the child summaries when they arrive in hosts that support that flow. Never silently imply background findings were incorporated when they were not.

**Dirty working tree pitfall:** if the repo has uncommitted changes, explicitly say whether findings are against HEAD, the dirty working tree, or both. Verification failures caused by current uncommitted edits are still valid findings, but plans must stamp the current commit and include drift checks for every changed in-scope file.

**Private/public release lane:** when a repo doubles as a private live deployment and is being prepared for public release, preserve the private system before writing scrub/package plans: create a non-git private copy, snapshot live/generated artifacts, align the live peer through its approved updater, then scrub public data. See `references/private-public-release-lane.md` for the exact sequence and verification pattern. When executing preservation/alignment plans, also use `references/operational-preservation-cross-checks.md`: verify preserved content, not just paths; compare live/generated artifacts by path set and SHA-256 where possible; prove the private copy is not tracked by the public repo; and check live alignment without rerunning mutating updaters unnecessarily. When executing the scrub, also use `references/private-public-scrub-verification.md`: scan tracked files via `git ls-files`, include root docs/agent guides, report token labels instead of values, and cross-check done criteria separately from passing tests.
...
Before writing anything: record `git rev-parse --short HEAD` — every plan stamps the commit it was written against (the executor uses it for drift detection). If the repo has a live/remote peer checkout that may be ahead (for example a local coding checkout plus a VPS/live checkout), and the user asks to align before planning, pause plan-writing and do the alignment first: inspect the peer checkout, commit only source-worthy changes there, push or relay-push to the canonical remote, stash local work, fast-forward pull locally, then restore only advisor plan files if needed. After alignment, update every plan's drift-check and planned-at SHA to the new HEAD and verify `HEAD`, `origin/main`, and the peer commit agree. See `references/live-checkout-alignment-before-plans.md` for the detailed pattern.

If `plans/` already exists from a previous run, **reconcile, don't duplicate**: read `plans/README.md`, keep numbering monotonic, skip findings already planned or listed as rejected, and mark superseded plans stale in the index. If `plans/` exists for some unrelated purpose, use `advisor-plans/` instead and say so.
...
### Phase 5 — Execute plans (only when explicitly asked)

When the user asks you to execute plans you wrote (or plans already present in `plans/`), you are now the executor. Follow the plan religiously — step by step, verifying at each step. Then:

**After all plans are executed, cross-check every plan's done criteria against actual code.** This is NOT the same as running verification commands. Verification commands can pass while done criteria are unmet — e.g. tests pass but a SQL column is never queried. For each plan:

1. Re-read the plan's done criteria checklist.
2. For each criterion, grep/source-dive the actual code to confirm the change landed.
3. Flag any gap — no matter how small — and fix it.
4. Re-run the full verification baseline after any fix.

**Do not claim plan completion after running verification commands alone.** The user can't see whether `packDir` is in the SQL query — only you can. Cross-check is mandatory before marking a plan DONE. See `references/cross-check-executed-plans.md` for the pattern used in the HaxJobs cleanup wave.
